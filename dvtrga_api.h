...
__declspec(dllimport) uint32_t *DvtrgaGetRawBuffer(void);

// Hybrid GPU API
__declspec(dllimport) int DvtrgaInitHybrid(uint32_t width, uint32_t height,
                                           const char *license_key);
__declspec(dllimport) void DvtrgaSwapBuffers(void);
__declspec(dllimport) void DvtrgaGpuSubmit(float *points, int count);
