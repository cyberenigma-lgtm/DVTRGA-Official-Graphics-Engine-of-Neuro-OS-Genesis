...
__declspec(dllexport) int DvtrgaInit(uint32_t width, uint32_t height,
  ...
  wc.lpszClassName = "DVTRGA_CPU_Class";
  wc.hCursor = LoadCursor(NULL, IDC_ARROW);
  RegisterClassA(&wc);

  // 2. Crear Ventana Dedicada (Popup visible)
  g_ctx.hwnd = CreateWindowExA(
      WS_EX_APPWINDOW | WS_EX_TOPMOST, "DVTRGA_CPU_Class",
      "DVTRGA CPU RASTERIZER - U9 DEDICATED", WS_POPUP | WS_VISIBLE, 0, 0,
      width, height, NULL, NULL, wc.hInstance, NULL);

  if (!g_ctx.hwnd) {
    // Fallback a Desktop si falla ventana
    g_ctx.hwnd = GetDesktopWindow();
  }

  // 3. Obtener DC de la ventana
...
__declspec(dllexport) void DvtrgaPresent(void) {
  ...
  // "TURBO" Mode: Direct output without GDI Object overhead
  // Bypass CreateCompatibleDC/BitBlt chain
  BITMAPINFO bmi = {0};
  bmi.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
  bmi.bmiHeader.biWidth = g_ctx.width;
  bmi.bmiHeader.biHeight = -(int32_t)g_ctx.height; // Top-down
  bmi.bmiHeader.biPlanes = 1;
  bmi.bmiHeader.biBitCount = 32;
  bmi.bmiHeader.biCompression = BI_RGB;

  SetDIBitsToDevice(g_ctx.hdc, 0, 0, g_ctx.width, g_ctx.height, 0, 0, 0,
                    g_ctx.height, g_ctx.pBits, &bmi, DIB_RGB_COLORS);
}
...
// Obtener puntero al buffer (Alta Velocidad)
__declspec(dllexport) uint32_t *DvtrgaGetRawBuffer() {
  if (g_ctx.initialized)
    return (uint32_t *)g_ctx.pBits;
  return NULL;
}
